function renderIngredients(){
  ...
  $('ingredientRows').innerHTML=matches.length
    ? matches.map(i=>`<tr><td><strong>${esc(i.name)}</strong></td><td>${esc(i.supplier||'-')}</td><td>${Number(i.purchase_quantity)} ${esc(i.purchase_unit)}</td><td>${money(i.purchase_price)}</td><td>${pct(i.yield_percent)}</td><td>${money(unitCost(i))}/${esc(i.base_unit)}</td><td><div class="action-row"><button class="mini-btn" onclick="editIngredient('${i.id}')">编辑</button><button class="mini-btn danger-btn" onclick="deleteIngredient('${i.id}')">删除</button></div></td></tr>`).join('')
...
(function setupIngredientSearch(){
  ...
  if(view&&!$('ingredientSearchInput')){
    ...
    if(panel&&tableWrap){
      ...
      wrap.innerHTML='<input id="ingredientSearchInput" type="search" placeholder="搜索原材料或供应商，例如：味精 / Yummy" style="width:100%;font-size:16px;padding:13px 14px" />';
