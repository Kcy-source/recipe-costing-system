(function setupIngredientBilingual(){
  ...
  renderIngredients=function(){
    ...
    document.getElementById('ingredientRows').innerHTML=matches.length
      ? matches.map(i=>`<tr><td><strong>${esc(i.name)}</strong>${i.name_en?`<br><span class="muted">${esc(i.name_en)}</span>`:''}</td><td>${esc(i.category||'-')}</td><td>${esc(i.supplier||'-')}</td><td>${Number(i.purchase_quantity)} ${esc(i.purchase_unit)}</td><td>${money(i.purchase_price)}<br><span class="muted">GST ${pct(i.gst_percent||0)} · 净 ${money(Number(i.gst_percent||0)>0?Number(i.purchase_price||0)/(1+Number(i.gst_percent)/100):Number(i.purchase_price||0))}</span></td><td>${pct(i.yield_percent)}</td><td>${money(unitCost(i))}/${esc(i.base_unit)}</td><td><div class="action-row"><button class="mini-btn" onclick="editIngredient('${i.id}')">编辑</button><button class="mini-btn danger-btn" onclick="deleteIngredient('${i.id}')">删除</button></div></td></tr>`).join('')
